            ...
            <template v-else>
              ...
              >
                ...
                <div class="relative shrink-0 flex items-center">
                  <button
                    type="button"
                    class="rounded-lg p-xs hover:bg-gray-25/80 focus:outline-none flex items-center justify-center"
                    :ref="(el) => setChatSessionMenuTriggerRef(chat.id, el)"
                    @click.stop="toggleChatSessionMenu(chat.id)"
                  >
                    <svg
                      class="h-4 w-4 rotate-90 text-gray-400 hover:text-gray-600 shrink-0"
                      fill="currentColor"
                      viewBox="0 0 24 24"
                      aria-hidden="true"
                    >
...
const chatSessionForOpenMenu = computed(() => {
  ...
  return props.chats.find((s) => String(s.id) === String(id)) ?? null
})
