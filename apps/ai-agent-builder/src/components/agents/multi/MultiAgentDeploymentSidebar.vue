...
import { regenerateDeploymentApiKey } from '../../../services/agents/deployment.js'
...
const endpoint = computed(() => {
  ...
  const groupId = deployment?.group_id || props.group?.id
  if (!groupId || String(raw).includes('/group_id=')) return raw

  const separator = String(raw).endsWith('/') ? '' : '/'
  return `${raw}${separator}group_id=${groupId}`
})
...
async function loadDeployments() {
  ...
  try {
    const result = await fetchGroupDeployment(props.group.id)
...
watch(
  () => [props.group?.id, props.isOpen],
  ([groupId, isOpen]) => {
    if (groupId && isOpen) loadDeployments()
  },
  { immediate: true }
)
...
async function handleCreateDeployment() {
  ...
  try {
    await createGroupDeployment(props.group.id, {
