<template>
  <div class="flex-1 bg_primary_color px-3xl py-12xl md:px-10xl lg:py-10xl">
    ...
    <!-- Products list -->
    <div v-else-if="hasProducts" class="w-full">
      <header class="mb-8xl bg_secondary_color rounded-lg p-6xl">
        <h1 class="heading_h6_bold primary_text_color">Products</h1>
        ...
        >
          <div class="relative w-full sm:max-w-md">
            ...
            <input
              ...
              class="w-full rounded-lg border primary_border_color bg_secondary_color py-2.5 pl-10 pr-3 label_2_medium primary_text_color outline-none focus:border-black-400"
              autocomplete="off"
            />
          </div>

          <div class="flex flex-wrap items-center gap-3">
            <div class="relative inline-block" data-product-list-filter>
              <button
                type="button"
                class="product-filter-trigger inline-flex min-w-[7.5rem] items-center justify-center gap-md rounded-xl bg_secondary_color px-xl py-xl label_1_semibold secondary_text_color transition-all"
                :class="
                  showFilterMenu
                    ? 'border-[3px] border-[#E6E8EB]'
                    : 'regular_border_color hover:bg-gray-25'
                "
              ...
              >
                Filter
                <img :src="FilterIcon" alt="" class="h-4 w-4 shrink-0 opacity-80" />
              </button>
              <div
                v-if="showFilterMenu"
                class="product-filter-menu absolute right-0 top-full z-20 mt-2 min-w-[172px] overflow-hidden rounded-2xl bg_secondary_color"
                role="listbox"
                :aria-activedescendant="`product-filter-${statusFilter}`"
                @click.stop
              >
                <template v-for="(option, index) in statusFilterOptions" :key="option.value">
                  <button
                    :id="`product-filter-${option.value}`"
                    type="button"
                    role="option"
                    :aria-selected="statusFilter === option.value"
                    class="flex w-full items-center justify-center px-3xl py-xl label_2_medium primary_text_color transition-colors hover:bg-gray-25"
                    :class="statusFilter === option.value ? 'bg-info-50' : ''"
                    @click="selectStatusFilter(option.value)"
                  >
                    {{ option.label }}
                  </button>
                  <div
                    v-if="index < statusFilterOptions.length - 1"
                    class="h-px w-full bg-gray-25"
                    aria-hidden="true"
                  />
                </template>
              </div>
            ...
            <button
              ...
              class="inline-flex items-center gap-md primary_button px-xl py-md label_1_semibold"
            >
      ...
      <div class="grid grid-cols-1 gap-5 lg:grid-cols-2">
        <article
          ...
          :key="product.id"
          :data-status="isProductActive(product)"
          :class="[
            'rounded-2xl border primary_border_color bg_secondary_color p-5xl shadow-sm transition-shadow',
            isProductActive(product)
              ? 'cursor-pointer hover:shadow-md'
              : 'cursor-not-allowed opacity-60',
          ]"
          :role="isProductActive(product) ? 'button' : undefined"
          :tabindex="isProductActive(product) ? 0 : -1"
          :aria-disabled="!isProductActive(product)"
          @click="onProductCardClick(product, $event)"
          @keydown.enter.prevent="onProductCardKeyActivate(product)"
          @keydown.space.prevent="onProductCardKeyActivate(product)"
        >
          <div class="flex items-start justify-between gap-3">
            ...
            <div class="relative shrink-0" data-product-card-menu>
              ...
              <div
                ...
                class="absolute right-0 z-10 mt-2 w-[min(calc(100vw-2rem),210px)] min-w-[200px] overflow-hidden rounded-xl border primary_border_color bg_secondary_color shadow-lg"
                @click.stop
              >
                <div class="flex items-center justify-between gap-3 px-4 py-3.5">
                  ...
                  <button
                    ...
                    class="inline-flex h-7 w-12 shrink-0 cursor-pointer items-center rounded-full px-[3px] transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500/40 disabled:cursor-not-allowed disabled:opacity-60"
                    :class="
                      isProductActive(product)
                        ? 'justify-end bg-green-300'
                        : 'justify-start bg-gray-50'
                    "
                ...
                <button
                  type="button"
                  class="flex w-full items-center justify-between gap-3 px-4 py-3.5 text-left label_2_medium primary_text_color transition-colors hover:bg-gray-25 disabled:cursor-not-allowed disabled:opacity-50"
          ...
          <div class="mt-6xl grid grid-cols-1 gap-6 sm:grid-cols-3 sm:gap-4">
            <div class="border-r border-[#F3F4F5]">
            ...
            <div class="min-w-0 border-r border-[#F3F4F5]">
...
/** Optimistic toggle overrides while PATCH is in flight. */
...
const filteredProducts = computed(() => {
  ...
  if (statusFilter.value === "active") {
    list = list.filter((p) => isProductActive(p));
...
function isProductActive(product) {
  const id = product?.id;
  if (id != null && activeOverrides.value[id] !== undefined) {
...
async function toggleProductActive(product) {
  const id = product?.id;
  if (!id || togglingActiveId.value) return;

  const previous = isProductActive(product);
  const next = !previous;

  activeOverrides.value = { ...activeOverrides.value, [id]: next };
  const idx = products.value.findIndex((p) => p.id === id);
  if (idx >= 0) {
    products.value[idx] = { ...products.value[idx], isActive: next };
  }

  togglingActiveId.value = id;
  try {
    await api.patch(`/api/products/${encodeURIComponent(id)}`, { isActive: next });
    const overrides = { ...activeOverrides.value };
    delete overrides[id];
    activeOverrides.value = overrides;
  } catch (e) {
    activeOverrides.value = { ...activeOverrides.value, [id]: previous };
    if (idx >= 0) {
      products.value[idx] = { ...products.value[idx], isActive: previous };
    }
    console.error("Toggle product active failed:", e);
    window.alert(
      e?.response?.data?.message || e?.message || "Could not update product status"
    );
  } finally {
...
function onProductCardKeyActivate(product) {
  if (!isProductActive(product)) return;
  emitSelectProduct(product);
}
...
function onDocClick(e) {
  ...
  if (!e.target.closest("[data-product-list-filter]")) {
    closeFilterMenu();
  }
}
...
async function fetchProducts() {
  ...
  try {
    ...
    products.value = list.map(normalizeProduct).filter(Boolean);
  } catch (e) {
...
.product-filter-trigger:focus-visible {
  outline: none;
}
</style>
