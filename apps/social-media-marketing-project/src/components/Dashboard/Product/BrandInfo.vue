<template>
  ...
  >
      ...
      <div class="mb-4">
        <div class="lg:w-1/2">
          ...
          <div class="flex-1 mt-6xl">
            ...
            <div class="mt-6xl">
              <label class="label_2_semibold block primary_text_color"> Website URL </label>
              <input
                :value="getWebsiteUrl()"
            ...
            <div class="mt-6xl">
              ...
              <button
                ...
                class="label_2_bold primary_text_color underline"
              >
    ...
    >
      <!-- Modal -->
      <div
        class="relative w-full max-w-xl bg_secondary_color rounded-2xl shadow-2xl overflow-hidden p-6"
        @click.stop
      >
        <!-- Title -->
        <h2 class="heading_h6_bold primary_text_color">
        ...
        <div
          ...
          :class="[
            ...
            isDragOver ? 'border-1 border-dashed border-gray-500 bg_secondary_color' : 'border-[1px] border-dashed border-gray-500'
          ]"
        ...
        <div
          v-if="fileError"
          class="mt-4 flex items-center justify-between bg-error-50 rounded-lg px-3xl py-xl"
        >
...
const removeFile = (index) => {
  ...
  hasFiles.value = fileList.value.some((f) => f.status === "completed");
...
defineExpose({
  ...
  /** Completed uploads as `File` instances (for `POST /api/products` multipart `pdf`). */
  getCompletedFiles() {
    return fileList.value
      .filter((f) => f.status === "completed" && f.file instanceof File)
      .map((f) => f.file);
  },
});
